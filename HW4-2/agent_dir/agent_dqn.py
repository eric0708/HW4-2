  ...
  def build_net(self, s, var_scope):
    ...
    with tf.variable_scope(var_scope):
      with tf.variable_scope('conv1'):
        W1 = self.init_W(shape=[5, 5, 4, 32])
        b1 = self.init_b(shape=[32])
        conv1 = self.conv2d(s, W1, strides=2)
        h_conv1 = tf.nn.relu(tf.nn.bias_add(conv1, b1))

      with tf.variable_scope('conv2'):
        W2 = self.init_W(shape=[5, 5, 32, 64])
        b2 = self.init_b(shape=[64])
        conv2 = self.conv2d(h_conv1, W2, strides=2)
        h_conv2 = tf.nn.relu(tf.nn.bias_add(conv2, b2))

      with tf.variable_scope('conv3'):
        W3 = self.init_W(shape=[3, 3, 64, 128])
        b3 = self.init_b(shape=[128])
        conv3 = self.conv2d(h_conv2, W3, strides=3)
        h_conv3 = tf.nn.relu(tf.nn.bias_add(conv3, b3))

      h_flatten = tf.reshape(h_conv3, [-1, 4608])
      
      with tf.variable_scope('fc1'):
        W_fc1 = self.init_W(shape=[4608, 512])
  ...
  def train(self):
    ...
    for episode in pbar:
      ...
      for s in range(self.args.max_num_steps):
        ...
        self.storeTransition(obs, action, reward_clip, obs_, done)
      ...
      if episode % self.args.num_eval == 0 and episode != 0:
        ...
        print("\n[Train] Avg Reward: " + "{:.2f}".format(avg_reward_train) + ", Avg Episode Length: " + "{:.2f}".format(avg_episode_len_train))
